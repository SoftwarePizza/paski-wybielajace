...
{block name="input"}
    {if $input.type == 'select_image'}
        ...
        <p>
            <a href="filemanager/dialog.php?type=1&amp;field_id={$input.name|escape:'html':'UTF-8'}" class="btn btn-default iframe-upload"  data-input-name="{$input.name|escape:'html':'UTF-8'}" type="button">{l s='Image selector' mod='x13popup'} <i class="icon-angle-right"></i></a>
        </p>
    {else}
        {$smarty.block.parent}
    {/if}
