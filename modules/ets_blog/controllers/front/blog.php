...
class Ets_blogBlogModuleFrontController extends ModuleFrontController
{
    ...
    public function submitCommentPost()
    {
        ...
        $id_post = (int)Tools::getValue('id_post');
        if(!$subject)
            $this->errors[] = $this->module->l('Temat komentarza jest wymagany','blog');
        else
        {
            if(Tools::strlen($subject) < 10)
                $this->errors[] = $this->module->l('Temat komentarza musi mieć przynajmniej 20 znaków','blog');
            if(Tools::strlen($subject) >300)
                $this->errors[] = $this->module->l('Temat komentarza nie może być dłuższy niż 300 znaków','blog');
            if(!Validate::isCleanHtml($subject,false))
                $this->errors[] = $this->module->l('Błąd tematu komentarza','blog');
        }
        if(!$comment)
            $this->errors[] = $this->module->l('Treść komentarza jest wymagana','blog');
        else
        {
            if(Tools::strlen($comment) < 20)
                $this->errors[] = $this->module->l('Treść komentarza musi mieć przynajmniej 20 znaków','blog');
            if(!Validate::isCleanHtml($comment,false))
                $this->errors[] = $this->module->l('Błędna treści komentarza','blog');
            if(Tools::strlen($comment) >2000)
                $this->errors[] = $this->module->l('Treść komentarza nie może być dłuższa niż 2000 znaków','blog');
        }
        if(!(int)$this->context->cookie->id_customer)
        {
            $name_customer = Tools::getValue('name_customer');
            if(!$name_customer)
                $this->errors[] = $this->module->l('Imie jest wymagane','blog');
        ...
        if(!$this->errors)
        {
            ...
            if($commentObj->add())
            {
                $this->context->cookie->_success = $this->module->l('Wysłano komentarz ','blog');
                if($commentObj->approved)
                    $this->context->cookie->_success .= $this->module->l('i zatwierdzono','blog');
                else
                    $this->context->cookie->_success .= $this->module->l('i czeka na zatwierdzenie','blog');
