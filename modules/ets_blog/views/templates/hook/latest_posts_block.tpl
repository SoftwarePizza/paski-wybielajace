...
*}
{*wyłaczony najnowszy post po lewej*}
{if $posts && false }
    <div class="block ets_block_latest {$ETS_BLOG_RTL_CLASS|escape:'html':'UTF-8'} {if isset($page) && $page}page_{$page|escape:'html':'UTF-8'}{else}page_blog{/if} ets_block_slider">
        <h4 class="title_blog title_block">{l s='Ostatnie artykuły ' mod='ets_blog'}</h4>
                        ...
                        <div class="ets-blog-latest-post-content">
                            ...
                            {if $allowComments || $show_views || $allow_like} 
                                <div class="ets-blog-latest-toolbar">
                                    {if $show_views}
                                        <span class="ets-blog-latest-toolbar-views">{$post.click_number|intval} {if $post.click_number!=1}<span>{l s='wyświeleń' mod='ets_blog'}</span>{else}<span>{l s='wyświetlenie' mod='ets_blog'}</span>{/if}</span> 
                                    {/if}   
                                    {if $allowComments && $post.comments_num>0}
                                        ...
                                        {l s='komnetarzy' mod='ets_blog'}</span>{else}<span>{l s='' mod='ets_blog'}</span>{/if}</span> 
                                    {/if}                                 
                                    {if $allow_like}
                                        <span title="{if $post.liked}{l s='Polubiono' mod='ets_blog'}{else}{l s='Polub ten artykuł' mod='ets_blog'}{/if}" class="ets-blog-like-span ets-blog-like-span-{$post.id_post|intval} {if $post.liked}active{/if}"  data-id-post="{$post.id_post|intval}">                        
                                            <span class="ben_{$post.id_post|intval}">{$post.likes|intval}</span>
                                            <span class="blog-post-like-text blog-post-like-text-{$post.id_post|intval}">{l s='Polubiono' mod='ets_blog'}</span>
