...
<div class="ets_blog_layout_{$blog_layout|escape:'html':'UTF-8'} ets-blog-wrapper-detail" itemscope itemType="http://schema.org/newsarticle">
                ...
                <div class="ets-blog-latest-toolbar">
                    ...
                    {if $show_date}
                        ...
                        <span class="post-date">
                            <span class="be-label">{l s='Opublikowano: ' mod='ets_blog'}: </span>
                    ...
                    {if $show_author && isset($blog_post.employee) &&  $blog_post.employee}
                        <div class="author-block" itemprop="author" itemscope itemtype="http://schema.org/Person">
                            <span class="post-author-label">{l s='Przez ' mod='ets_blog'}</span>
            ...
            <div class="ets-blog-wrapper-comment"> 
                {if $allowComments}
                    <div class="ets_comment_form_blog">
                        <h4 class="title_blog">{l s='Zostaw komentarz' mod='ets_blog'}</h4>
                        <div class="ets-blog-form-comment" id="ets-blog-form-comment">                   
                                ...
                                <form action="#ets-blog-form-comment" method="post">
                                    ...
                                    {if !$hasLoggedIn} 
                                        <div class="blog-comment-row blog-name">
                                            <label for="bc-name">{l s='Imie' mod='ets_blog'}</label>
                                        ...
                                        <div class="blog-comment-row blog-email">
                                            <label for="bc-email">{l s='Mail' mod='ets_blog'}</label>
                                    ...
                                    <div class="blog-comment-row blog-title">
                                        <label for="bc-subject">{l s='Temat ' mod='ets_blog'}</label>
                                    ...
                                    <div class="blog-comment-row blog-content-comment">
                                        <label for="bc-comment">{l s='Treść ' mod='ets_blog'}</label>
                                            ...
                                            <div class="blog-rate-capcha">
                                                {if $allow_rating}                            
                                                    <div class="blog-rate-post">
                                                        <label>{l s='Ocena: ' mod='ets_blog'}</label>
                                        ...
                                        <div class="blog-submit-form">                                                    
                                            {if !Configuration::get('ETS_BLOG_DISPLAY_GDPR_NOTIFICATION')}
                                                <div class="blog-submit">
                                                    <input class="button" type="submit" value="{l s='Zapisz' mod='ets_blog'}" name="bcsubmit" />
                                                </div>
                                            {/if}   
                                        </div>            
                                    ...
                                    <div class="blog-comment-row">                                            
                                        <div class="blog-submit-form">
                                            {if Configuration::get('ETS_BLOG_DISPLAY_GDPR_NOTIFICATION')}
                                                ...
                                                <div class="blog-submit">
                                                    <input class="button" type="submit" disabled="disabled" value="{l s='Zapisz' mod='ets_blog'}" name="bcsubmit" />
                                                </div>
                                            {/if}   
                                        </div>  
                                    </div>                                            
                            ...
                            {else}
                                <p class="alert alert-warning">{l s='Zaloguj się aby skomentować' mod='ets_blog'}</p>
                            {/if}
                        </div> 
                    </div>
                    {if count($comments)}
                        <div class="ets_blog-comments-list">
                        <h4 class="title_blog">{l s='Komentarze' mod='ets_blog'}</h4>
                            <ul id="blog-comments-list" class="blog-comments-list">
                                {foreach from=$comments item='comment'}
                                        ...
                                        <div class="ets-blog-detail-comment">
                                            ...
                                            {if $comment.name}<span class="comment-by">{l s='Przez: ' mod='ets_blog'}<b>{ucfirst($comment.name)|escape:'html':'UTF-8'}</b></span>{/if}
                                            <span class="comment-time"><span>{l s='Opublikowano:' mod='ets_blog'} </span>{date($date_format,strtotime($comment.date_add))|escape:'html':'UTF-8'}</span>
                                            {if $allow_rating && $comment.rating > 0}
                                                <div class="comment-rating" >
                                                    <span>{l s='Oceny: ' mod='ets_blog'}</span>
                                                ...
                                                {foreach $comment.replies item='reply'}
                                                    <p class="comment-reply">
                                                        <span class="ets-blog-replied-by">
                                                            {l s='Odpowiada: ' mod='ets_blog'}
                                                            <span class="ets-blog-replied-by-name">
                                                                {ucfirst($reply.name)|escape:'html':'UTF-8'}
                                                            </span>              
                                                        </span>
                                                        <span class="comment-time"><span>{l s='Opublikowano' mod='ets_blog'} </span>{date($date_format,strtotime($reply.date_add))|escape:'html':'UTF-8'}</span>
                                                ...
                                                {else}
                                                    {if isset($comment.reply) && $comment.reply}
                                                        <form class="form_reply_comment" action="#blog_comment_line_{$comment.id_comment|intval}" method="post">
                                                            ...
                                                            <textarea name="reply_comwent_text" placeholder="{l s='Wprowadź wiadomość' mod='ets_blog'}">{$reply_comwent_text nofilter}</textarea>
                            ...
                            {if isset($link_view_all_comment)}
                                <div class="blog_view_all_button">
                                    <a href="{$link_view_all_comment|escape:'html':'UTF-8'}" class="view_all_link">{l s='Wszystkie komentarze' mod='ets_blog'}</a>
                                </div>
                            {/if}
                        </div>               
                    {/if}
 ...
 {if $blog_post.related_posts}
            <div class="ets-blog-related-posts ets_blog_related_posts_type_{if $blog_related_posts_type}{$blog_related_posts_type|escape:'html':'UTF-8'}{else}default{/if}">
                <h4 class="title_blog">{l s='Zobacz też' mod='ets_blog'}</h4>
